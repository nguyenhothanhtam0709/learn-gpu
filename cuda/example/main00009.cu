...
__global__ void count_equal_kernel(const int *__restrict__ input,
                                   ...
                                   const int K)
{
    __shared__ int blockCount[THREADS_PER_BLOCK]; // Shared within block
    ...
    blockCount[threadIdx.x] = localCount;
    __syncthreads();

    for (int s = blockDim.x / 2; s > 0; s >>= 1)
    {
        if (threadIdx.x < s)
            blockCount[threadIdx.x] += blockCount[threadIdx.x + s];
        __syncthreads();
    }

    if (0 == threadIdx.x)
        atomicAdd(output, blockCount[0]);
}
